...
async function convertReminderPrefrences() {
  const reminderPrefs = getSetting("reminders");
  const alarmTimes = reminderPrefs.split("|")
  let returnString = ""
...
LOCATION:${shift.orgpath ||  getSetting("defaultLocation")}
DESCRIPTION:${generateShiftDescription(shift)}
${getSetting('reminders') ? convertReminderPrefrences() : null}
END:VEVENT
`;
      })
    }
